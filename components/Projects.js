...
export default function Projects() {
  ...
  const projectApi = async () => {
      let result = await fetch(`https://reassuring-fish-5cf727fbdc.strapiapp.com/api/projects?populate=*`);
                ...
                flirt.map((item) => {
                  return <Col key={item.id}>
                    <div className="my-blog-card">
                      <Image src={`${item.attributes.image.data.attributes.formats.thumbnail.url}`} alt="blog 1" width={350} height={250} />
